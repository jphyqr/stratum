// app/(docs)/product-stream/_components/ContextInput.tsx
"use client";
...
export function ContextInput({ onValidExtraction }: ContextInputProps) {
  ...
  const validateAndParseInput = useCallback((text: string) => {
    try {
    ...
    <Card className="p-4">
        ...
        {validation.summary && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">
              Found: {validation.summary.workItems} items, {validation.summary.decisions} decisions
            </span>
          </div>
        )}
      </div>
      ...
      <div className="relative">
        <textarea
          ...
          className="min-h-[400px] w-full rounded-md border border-input bg-background p-3 text-sm font-mono"
