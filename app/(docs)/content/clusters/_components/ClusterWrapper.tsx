...
// app/(docs)/content/_components/ClusterWrapper.tsx
export default async function ClusterWrapper({ promise, projectId }: { promise: Promise<Response>; projectId: string }) {
    ...
    return (
      ...
      <ClusterCard cluster={cluster} projectId={projectId}>
        ...
        {
            cluster._count.keywords ==  0? <div>Generate Keywords</div>
            :
            <KeywordTree 
            promise={
        ...
        {
            cluster._count.briefs ==  0? <div>Generate Briefs</div>
            :
            <BriefList 
            promise={
                makeExtensionRequest(
