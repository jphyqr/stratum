// app/(docs)/content/_components/useFormProgress.ts
...
export function useFormProgress(form: UseFormReturn<ContentStrategyFormData>) {
  ...
  const sections = {
    ...
    "Audience": !!(
      ...
      values.targetPersonas.every(persona => 
        persona.name &&
        persona.role &&
  ...
  const total = Math.round((completedSections / Object.keys(sections).length) * 100)
