// app/(docs)/content/_components/sections/TargetAudienceSection.tsx
...
export function TargetAudienceSection({ form }: TargetAudienceSectionProps) {
  ...
  const removePersona = (index: number) => {
    const newPersonas = personas.filter((_persona: any, i: number) => i !== index)
    form.setValue("targetPersonas", newPersonas, { shouldDirty: true })
  }

  const addListItem = (personaIndex: number, listType: 'challenges' | 'goals' | 'preferences') => {
    if (newItem.personaIndex !== personaIndex || newItem.type !== listType) return
    
    const value = newItem.value.trim()
    if (!value) return

    const fieldName = listType === 'preferences' ? 'contentPreferences' : listType
    const currentList = form.watch(`targetPersonas.${personaIndex}.${fieldName}`) || []
  ...
  const removeListItem = (personaIndex: number, listType: string, itemIndex: number) => {
    const fieldName = listType === 'preferences' ? 'contentPreferences' : listType
    const currentList = form.watch(`targetPersonas.${personaIndex}.${fieldName}`)
    const newList = currentList.filter((_item: string, i: number) => i !== itemIndex)
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    value={newItem.personaIndex === index && newItem.type === 'challenges' ? newItem.value : ''}
                ...
                <div className="space-y-2">
                  {persona.challenges?.map((challenge: string, challengeIndex: number) => (
                    <div key={challengeIndex} className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => removeListItem(index, 'challenges', challengeIndex)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    value={newItem.personaIndex === index && newItem.type === 'goals' ? newItem.value : ''}
                    onChange={(e) => setNewItem({
                      personaIndex: index,
                      type: 'goals',
                      value: e.target.value
                  ...
                  {persona.goals?.map((goal: string, goalIndex: number) => (
                    <div key={goalIndex} className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => removeListItem(index, 'goals', goalIndex)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    value={newItem.personaIndex === index && newItem.type === 'preferences' ? newItem.value : ''}
                    onChange={(e) => setNewItem({
                      personaIndex: index,
                      type: 'preferences',
                      value: e.target.value
                ...
                <div className="space-y-2">
                  {persona.contentPreferences?.map((preference: string, preferenceIndex: number) => (
                    <div key={preferenceIndex} className="flex items-center gap-2">
                      <div className="flex-1 rounded-md border bg-muted/50 px-3 py-2">
                        {preference}
                      </div>
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => removeListItem(index, 'preferences', preferenceIndex)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
