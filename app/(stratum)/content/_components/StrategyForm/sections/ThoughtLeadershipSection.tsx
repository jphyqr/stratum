// app/(stratum)/content/_components/sections/ThoughtLeadershipSection.tsx
...
export function ThoughtLeadershipSection({
  ...
  const addPosition = () => {
    if (newPosition.trim()) {
      form.setValue(
        "thoughtLeadership.keyPositions",
  ...
  const removePosition = (index: number) => {
    const newPositions = positions.filter(
      (_position: string, i: number) => i !== index
    )
    form.setValue("thoughtLeadership.keyPositions", newPositions, {
      shouldDirty: true,
    })
  }
  ...
  const addPerspective = () => {
    if (newPerspective.trim()) {
      form.setValue(
        "thoughtLeadership.uniquePerspectives",
        [...perspectives, newPerspective.trim()],
        {
          shouldDirty: true,
        }
      )
  ...
  const removePerspective = (index: number) => {
    const newPerspectives = perspectives.filter(
      (_perspective: string, i: number) => i !== index
    )
    form.setValue("thoughtLeadership.uniquePerspectives", newPerspectives, {
      shouldDirty: true,
    })
  }
