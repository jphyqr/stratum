// app/(stratum)/content/_components/sections/IndustryContextSection.tsx
...
export function IndustryContextSection({ form }: IndustryContextSectionProps) {
  ...
  const removeTerm = (index: number) => {
    ...
    form.setValue("industryContext.terminology", newTerms, {
      shouldDirty: true,
    })
  }
  ...
  const addConcept = () => {
    if (newConcept.trim()) {
      form.setValue(
        "industryContext.concepts",
        [...concepts, newConcept.trim()],
        {
          shouldDirty: true,
        }
      )
  ...
  const removeConcept = (index: number) => {
    const newConcepts = concepts.filter(
      (_concept: string, i: number) => i !== index
    )
    form.setValue("industryContext.concepts", newConcepts, {
      shouldDirty: true,
    })
  }
